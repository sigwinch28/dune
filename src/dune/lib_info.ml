...
module Special_builtin_support = struct
  ...
  let to_dyn x =
    ...
    | Sites_locations info ->
      constr "Sites_locations" [ Sites_locations.to_dyn info ]
    ...
    sum
      ...
      ; ( "configurator"
        , let+ () = Dune_lang.Syntax.since Stanza.syntax (2, 3)
          ...
          Configurator info )
      ; ( "sites_locations"
          (* , let+ () = Dune_lang.Syntax.since Stanza.syntax (2, 5) (* moving
             dune's dune-project to 2.5 failed because of coq rules *) *)
  ...
  let encode t =
    ...
    | Sites_locations x ->
      Dune_lang.List
        (Dune_lang.atom "sites_locations" :: Sites_locations.encode x)
end
